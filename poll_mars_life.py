...
def main():
    ...
    if done == n:
        ...
        log(f"Final: {FINAL_OUT}")
        state["done"] = True
        state["final"] = FINAL_OUT
        with open(f"{WD}/state.json", "w") as f:
            json.dump(state, f, ensure_ascii=False, indent=2)
    else:
        log(f"Progress: {done}/{n}")
